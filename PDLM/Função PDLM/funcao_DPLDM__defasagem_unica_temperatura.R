DPLDM = function(Y, x, regressora1, regressora2, lags, lag_temperatura, d, fd_nivel, fd_temperatura, fd_phi, quantil_corte){
  ...
  H = ifelse(H > quantile(regressora2, quantil_corte), 1, 0)
  ...
  for(i in 1:time){
    amostra_theta[,,i] = t(t(amostra_theta[,,i])/c(1,DP,1))
  }
  ...
  amostra_eta = amostra_theta[,c(2:(n-1)),time]
  ...
  #amostra psi
  amostra_psi = amostra_theta[,n,time]
